...
def _anthropic_model_pricing(model: str) -> dict[str, float]:
    model_lower = model.lower().rsplit("/", 1)[-1]
    matches = [prefix for prefix in _ANTHROPIC_PRICING if model_lower.startswith(prefix)]
    if not matches:
        return {"input": 3.0, "output": 15.0}
    return _ANTHROPIC_PRICING[max(matches, key=len)]
...
def _cache_usage(usage: dict[str, Any]) -> dict[str, int]:
    ...
    return {
        ...
        "cache_write_5m_input_tokens": int(creation.get("ephemeral_5m_input_tokens") or 0),
        "cache_write_1h_input_tokens": int(creation.get("ephemeral_1h_input_tokens") or 0),
    }


def _anthropic_usage_cost(
    *, model: str, prompt_tokens: int, completion_tokens: int, usage: dict[str, Any]
) -> float:
    """Calculate Anthropic cost from its disjoint input/cache usage categories."""
    pricing = _anthropic_model_pricing(model)
    cache = _cache_usage(usage)
    read_tokens = cache["cached_input_tokens"]
    write_tokens = cache["cache_write_input_tokens"]
    write_5m = min(cache["cache_write_5m_input_tokens"], write_tokens)
    write_1h = min(cache["cache_write_1h_input_tokens"], max(write_tokens - write_5m, 0))
    unclassified_writes = max(write_tokens - write_5m - write_1h, 0)
    ...
    input_units = (
        ...
        + (write_5m * 1.25)
        + (write_1h * 2.0)
        + (unclassified_writes * 1.25)
    )
    return (
        input_units * pricing["input"] + max(completion_tokens, 0) * pricing["output"]
    ) / 1_000_000
...
class AnthropicProvider(BaseProvider):
    ...
    def _calculate_response_cost(
        ...
        *,
        ...
        """Use Anthropic cache categories when present, preserving LiteLLM otherwise."""
        cache = _cache_usage(usage)
        if cache["cached_input_tokens"] or cache["cache_write_input_tokens"]:
            return _anthropic_usage_cost(
                model=model,
