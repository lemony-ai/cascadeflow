...
class UserTier(BaseModel):
    ...
    latency: LatencyProfile = Field(
        ...,
        description="Latency profile for this tier"
    )

    # ===== OPTIMIZATION WEIGHTS =====
    optimization: OptimizationWeights = Field(
        ...,
        description="Multi-factor optimization weights"
    )
    ...
    allowed_models: List[str] = Field(
        ...
        description="Models this tier can use (* = all)"
    )
    preferred_models: List[str] = Field(
        default_factory=list,
        description="Prefer these models when available"
    )
    exclude_models: List[str] = Field(
        default_factory=list,
        description="Never use these models"
    )

    # Keep for backwards compatibility
    excluded_models: List[str] = Field(
        default_factory=list,
        description="Excluded models (alias for exclude_models)"
    )

    # ===== OPTIMIZATION FLAGS =====
    optimize_simple_queries: bool = Field(
        True,
        description="Optimize even simple queries for cost"
    )
    ...
    local_only: bool = Field(
        False,
        description="Only use local models (no cloud)"
    )
...
class WorkflowProfile(BaseModel):
    ...
    latency_override: Optional[LatencyProfile] = Field(
        None,
        description="Override tier latency settings"
    )
    optimization_override: Optional[OptimizationWeights] = Field(
        None,
        description="Override tier optimization weights"
    )
    max_budget_override: Optional[float] = Field(
        None,
        description="Override tier max budget"
    )
    quality_threshold_override: Optional[float] = Field(
        None,
        description="Override tier quality threshold"
    )

    # ===== MODEL CONTROL =====
    force_models: Optional[List[str]] = Field(
        None,
        description="Force use of specific models only"
    )
    preferred_models: List[str] = Field(
        default_factory=list,
        description="Prefer these models"
    )
    exclude_models: List[str] = Field(
        default_factory=list,
        description="Exclude these models"
    )
...
EXAMPLE_WORKFLOWS = {
    ...
    "realtime": WorkflowProfile(
        ...
        latency_override=LatencyProfile(
            max_total_ms=800,
            max_per_model_ms=600,
            prefer_parallel=True,
            skip_cascade_threshold=700
        ),
