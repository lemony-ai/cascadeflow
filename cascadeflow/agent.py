...
class CascadeAgent:
    ...
    async def stream_events(
        ...
        **kwargs,
        ...
        # Keep draft-decision capture for compatibility while using record().
        _stream_draft_decision = None
        result_data = None
        ...
        if use_cascade and streaming_manager:
            async for event in streaming_manager.stream(
                ...
                **kwargs,
                ...
                event_type_value = getattr(getattr(event, "type", None), "value", None)
                if event_type_value == "draft_decision":
                    _stream_draft_decision = getattr(event, "data", None)
                elif event_type_value == "complete" and event.data:
            ...
            if streaming_manager:
                async for event in streaming_manager.stream(
                    ...
                    **kwargs,
                    ...
                    if getattr(event.type, "value", None) == "complete" and event.data:
                        result_data = event.data.get("result")
                    yield event
            else:
        ...
        # Record telemetry from captured stream result data.
        # If the stream ended before COMPLETE but draft decision exists, synthesize
        # minimal result data so acceptance/rejection counters remain correct.
        if result_data:
            result = self._dict_to_result(result_data)
        elif use_cascade and isinstance(_stream_draft_decision, dict):
            result = self._dict_to_result(
                {
                    ...
                    "total_cost": 0.0,
                    "latency_ms": 0.0,
                    "draft_accepted": bool(_stream_draft_decision.get("accepted", False)),
        ...
        # Record telemetry from captured result data
        self.telemetry.record(
            result=result,
            routing_strategy=routing_strategy,
            complexity=complexity.value,
