...
from .complexity import QueryComplexity
from .config import ModelConfig, UserTier, WorkflowProfile, OptimizationWeights
...
class DomainDetector:
    ...
    DOMAIN_KEYWORDS = {
        "code": [
            ...
            "bug", "error", "debug", "implement", "algorithm", "api",
            "syntax", "compile", "runtime", "programming", "script",
            "variable", "loop", "if", "else", "return", "import",
            "react", "node", "django", "flask", "typescript", "c++",
            "rust", "go", "kotlin", "swift", "git", "github"
        ],
        "math": [
            "equation", "derivative", "integral", "calculate", "solve",
            "algebra", "calculus", "geometry", "trigonometry", "matrix",
            "probability", "statistics", "mean", "median", "variance",
            "standard deviation", "regression", "correlation", "theorem",
            "proof", "formula", "calculate", "compute"
        ],
        "data": [
            "dataframe", "pandas", "sql", "query", "database", "table",
            "csv", "excel", "analysis", "visualization", "plot", "chart",
            "numpy", "aggregate", "group by", "join", "merge", "filter",
            "transform", "pivot", "reshape", "clean data"
        ],
        "writing": [
            "write", "essay", "article", "blog", "content", "draft",
            "edit", "revise", "proofread", "grammar", "style", "tone",
            "paragraph", "introduction", "conclusion", "thesis",
            "persuasive", "narrative", "descriptive", "summary"
        ],
        "legal": [
            "contract", "agreement", "legal", "law", "clause", "terms",
            "liability", "compliance", "regulation", "statute", "court",
            "litigation", "plaintiff", "defendant", "jurisdiction"
        ],
        "medical": [
            "medical", "health", "disease", "symptom", "diagnosis",
            "treatment", "medication", "doctor", "patient", "clinical",
            "therapy", "prescription", "anatomy", "physiology"
        ],
        "finance": [
            "investment", "stock", "portfolio", "trading", "financial",
            "market", "equity", "bond", "dividend", "roi", "revenue",
            "profit", "balance sheet", "income statement", "valuation"
        ]
    }
    ...
    async def create_plan(
        ...
        if complexity == QueryComplexity.TRIVIAL:
            ...
            if (tier and tier.enable_speculative and
                ...
                max_latency = max_latency_ms or (
                    tier.latency.max_total_ms if tier else 10000
                )
                ...
                if combined_latency < max_latency:
                    ...
                    primary = None
                    reasoning = (
                        f"Moderate query → speculative "
                        f"({drafter.name} → {verifier.name})"
                    )
                else:
                    strategy = ExecutionStrategy.DIRECT_SMART
                    primary = best_model
                    reasoning = (
                        "Moderate query → direct "
                        "(speculative exceeds latency budget)"
                    )
            else:
                strategy = ExecutionStrategy.DIRECT_SMART
                primary = best_model
            ...
            if (tier and tier.enable_parallel and
                    len(scored_models) >= tier.parallel_race_count):
                strategy = ExecutionStrategy.PARALLEL_RACE
                primary = None
                race_models = [m for m, _, _ in scored_models[:tier.parallel_race_count]]
                reasoning = (
                    f"Complex query → parallel race "
                    f"({len(race_models)} models)"
                )
            else:
        ...
        plan = ExecutionPlan(
            strategy=strategy,
            primary_model=primary if strategy in [
            ...
            metadata={
                ...
                "top_3_models": [
                    {"name": m.name, "score": s, **meta}
                    for m, s, meta in scored_models[:3]
                ]
            }
        )
