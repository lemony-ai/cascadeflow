...
from cascadeflow.query_difficulty import QueryDifficultyEstimator
from cascadeflow.alignment_scorer import QueryResponseAlignmentScorer
...
class ConfidenceAnalysis:
    ...
    method_used: str  # 'logprobs', 'semantic', 'hybrid', 'multi-signal-hybrid', 'multi-signal-semantic'
...
class ProductionConfidenceEstimator:
    ...
    def estimate(
        ...
        if logprobs and len(logprobs) > 0:
            logprobs_conf = self._calculate_from_logprobs(
                logprobs, tokens, response
            )
        ...
        # ============================================================================
        # CRITICAL SAFETY CHECK: Alignment Floor (Phase 3 FIX)
        # ============================================================================
        ...
        #
        # Research basis:
        # - Off-topic responses typically have alignment < 0.30
        ...
        if alignment_score is not None and alignment_score < 0.30:
    ...
    def _analyze_semantic_quality(
        ...
        FIXED (2025-10-06): Replaced multiplicative penalties with additive
        ...
        strong_hedges = [
            ...
            "i don't have information", "i'm unable to",
            "uncertain", "unclear about"
        ]

        moderate_hedges = [
            "probably", "might be", "could be", "perhaps",
            "it seems", "appears to", "may", "possibly",
            "likely", "i think", "i believe"
        ]
        ...
        if word_count > 0:
            ...
            hedge_penalty = min(
                0.30,
                (strong_ratio * 0.05) + (moderate_ratio * 0.02)
            )
        else:
            hedge_penalty = 0
        ...
        evasive_patterns = [
            "it depends", "that's a complex question",
            "there are many factors", "it varies",
    ...
    def explain_confidence(
        ...
        if analysis.query_difficulty is not None:
            category = 'trivial' if analysis.query_difficulty < 0.3 else \
                'simple' if analysis.query_difficulty < 0.5 else \
                    'moderate' if analysis.query_difficulty < 0.7 else 'complex'
            lines.append(
                f"  Query difficulty: {analysis.query_difficulty:.2f} ({category})"
            )
        ...
        if analysis.alignment_score is not None:
            ...
            if analysis.alignment_floor_applied:
                ...
                lines.append(
                    f"  ⚠️  SAFETY: Alignment floor applied ({severity} off-topic)"
                )
                lines.append(
...
def update_base_provider_confidence(provider_instance: Any) -> None:
    ...
    original_method = provider_instance.calculate_confidence
