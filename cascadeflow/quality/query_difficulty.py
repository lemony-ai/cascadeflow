...
class QueryDifficultyEstimator:
    ...
    FACTUAL_PATTERNS = [
        ...
        'name a', 'name the', 'list', 'give me', 'tell me'
    ]

    EXPLANATORY_PATTERNS = [
        'why', 'how does', 'how do', 'explain briefly',
        'describe', 'what causes'
    ]

    ANALYTICAL_PATTERNS = [
        'analyze', 'compare', 'contrast', 'evaluate', 'assess',
        'examine', 'critique', 'discuss', 'argue'
    ]

    SYNTHETIC_PATTERNS = [
        'synthesize', 'design', 'propose', 'create', 'develop',
        'formulate', 'construct', 'devise'
    ]

    COMPLEXITY_MARKERS = [
        'implications', 'framework', 'methodology', 'theoretical',
        'paradigm', 'philosophical', 'epistemological', 'ontological',
        'dialectical', 'heuristic', 'normative', 'empirical',
        'phenomenological', 'hermeneutic', 'meta-analysis'
    ]

    CONDITIONAL_PATTERNS = [
        'if', 'suppose', 'assume', 'given that', 'provided that',
        'in the case', 'hypothetically', 'what would happen'
    ]
    ...
    def estimate(self, query: str, verbose: bool = False) -> float:
        ...
        if not query or len(query.strip()) < 2:
            return 0.0 if not verbose else DifficultyAnalysis(
                difficulty=0.0,
                category='trivial',
                features={},
    ...
    def _analyze_complexity_markers(self, query_lower: str) -> float:
        ...
        """
        marker_count = sum(
            1 for marker in self.COMPLEXITY_MARKERS
    ...
    def _analyze_multi_questions(self, query_lower: str) -> float:
        ...
        compound_markers = [
            ...
            'and then', 'as well as', 'along with'
        ]
...
if __name__ == "__main__":
    ...
    for query, expected in test_queries:
        ...
        print(f"{status} {query:50s} → {analysis.difficulty:.3f} ({analysis.category}, expected {expected})")
    ...
    if trivial_scores and simple_scores:
        ...
        print(f"  Separation:   {separation:.3f} {'✅ PASS' if separation > 0.1 else '❌ FAIL'} (target: >0.1)")