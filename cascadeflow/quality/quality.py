...
FIXED (Week 2 Day 4): COMPLETE INTEGRATION
...
# Import complexity detection
try:
    from .complexity import QueryComplexity, ComplexityDetector
    COMPLEXITY_AVAILABLE = True
except ImportError:
    COMPLEXITY_AVAILABLE = False
    logger.warning("complexity.py not available - using basic mode")

# Import alignment scorer (CRITICAL!)
try:
...
class ResponseAnalyzer:
    ...
    HEDGING_PHRASES = [
        ...
        'likely', 'probably', 'generally', 'usually', 'typically',
        'often', 'sometimes', 'can be', 'tends to', 'seems to',
        'appears to', 'suggests', 'indicates', 'implies',
        'i think', 'i believe', 'in my opinion', 'arguably',
        'somewhat', 'rather', 'relatively', 'fairly', 'quite',
        'to some extent', 'in some cases', 'it depends'
    ]

    # Strong uncertainty markers (worse than hedging)
    UNCERTAINTY_MARKERS = [
        "i don't know", "i'm not sure", "i cannot", "i can't",
        "unclear", "uncertain", "not confident", "no information",
        "don't have", "unable to", "cannot provide", "insufficient",
        "i apologize", "i'm sorry", "unfortunately", "not able to",
        "beyond my knowledge", "outside my expertise"
    ]
    ...
    def detect_hedging(content: str) -> Dict[str, Any]:
        ...
        hedging_count = sum(
            1 for phrase in ResponseAnalyzer.HEDGING_PHRASES
            if phrase in content_lower
        )

        has_severe = any(
            marker in content_lower
            for marker in ResponseAnalyzer.UNCERTAINTY_MARKERS
        )
    ...
    def analyze_specificity(content: str, complexity: str) -> Dict[str, Any]:
        ...
        has_examples = any(word in content_lower for word in ['example', 'for instance', 'such as', 'e.g.'])
        has_quotes = '"' in content or "'" in content
        has_references = any(word in content_lower for word in ['according to', 'research', 'study', 'source'])
        has_technical_terms = len(re.findall(r'\b[A-Z][a-z]+(?:[A-Z][a-z]+)+\b', content)) > 0

        vague_phrases = ['thing', 'stuff', 'something', 'various', 'many', 'some', 'several', 'often', 'usually']
    ...
    def detect_hallucinations(content: str) -> Dict[str, Any]:
        ...
        has_contradiction = bool(re.search(
            r'(however|but|although|though|yet).*\b(not|no|never)\b',
            content,
            re.IGNORECASE
        ))
        ...
        return {
            ...
            'risk_level': 'high' if len(suspicious_patterns) >= 2 else ('medium' if len(suspicious_patterns) == 1 else 'low')
        }
...
class QualityValidator:
    ...
    def validate(
        ...
        if complexity == 'trivial':
            ...
            checks['has_content'] = len(draft_content.strip()) >= self.config.min_length_thresholds.get(complexity, 10)
...
class ComparativeValidator:
    ...
    async def validate(
        ...
        try:
            ...
            reason = f"similarity {similarity:.2f} {'≥' if passed else '<'} {self.similarity_threshold}"
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'samples_per_complexity': {
                ...
                for complexity, history in self.history.items()
            }
        }
...
if __name__ == "__main__":
    ...
    for name, config in configs.items():
        ...
        if name == 'cascade':
            print(f"  → CASCADE-OPTIMIZED (Phase 2)")
            print(f"  → Target: 50-60% acceptance, 94-96% quality")
