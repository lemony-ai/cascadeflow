...
with cascadeflow.run(budget=0.50, compliance="pii_audit") as session:
...
async def validated_agent(query: str):
    # Within this agent, cascadeflow automatically:
    # - Blocks PII in outputs
    # - Validates factuality at min_confidence=0.8
    # - Checks consistency with tolerance=0.05
    # - Enforces latency_sla=2000ms
    # - Tracks tokens within budget of 500
...
@cascadeflow.agent(budget=0.50, compliance="no_pii_logs", kpi_weights={"quality": 0.8, "cost": 0.2}, kpi_targets={"quality": 0.85}, max_tool_calls=5)
...
with cascadeflow.run(budget=1.00, compliance="no_pii_logs") as session:
...
const agent = new CascadeAgent({
  models: [
    { name: 'gpt-4-turbo', provider: 'openai', cost: 0.00625 },
    { name: 'gpt-3.5-turbo', provider: 'openai', cost: 0.0005 },
    { name: 'claude-3-sonnet', provider: 'anthropic', cost: 0.003 },
  ],
  quality: { threshold: 0.85, useSemanticValidation: true },
  budget: { costLimit: 1.0, latencySLA: 3000 },
  compliance: { rules: ['no_pii_in_logs'] },
});

const draftResult = await agent.run('Analyze this dataset');
const verifyResult = await agent.run(`Verify: ${draftResult.output}`);

console.log(`Draft Model: ${draftResult.modelUsed}, Cost: $${draftResult.totalCost.toFixed(2)}`);
console.log(`Verify Model: ${verifyResult.modelUsed}, Quality: ${(draftResult.quality * 100).toFixed(1)}%`);
```
...
- [cascadeflow Documentation](https://cascadeflow.ai)
