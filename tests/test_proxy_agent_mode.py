            ...
            "tool_calls": [
                {
                    "id": "call_1",
                    "type": "function",
                    "function": {"name": "calculator", "arguments": json.dumps({"expression": "2+2"})},
                }
            ],
        }
...
def agent_proxy():
    proxy = RoutingProxy(
        agent=_FakeAgent(),
        config=ProxyConfig(host="127.0.0.1", port=0, allow_streaming=True, enable_classifier=False),
    )
    proxy.start()
    yield proxy
    proxy.stop()
...
def test_anthropic_agent_response_shape(agent_proxy):
    ...
    assert isinstance(data["content"], list) and data["content"]
...
def test_anthropic_agent_streaming(agent_proxy):
    url = f"http://{agent_proxy.host}:{agent_proxy.port}/v1/messages"
    payload = {"model": "claude-any", "messages": [{"role": "user", "content": "hi"}], "stream": True}
    with httpx.stream("POST", url, json=payload, timeout=5.0) as resp:
        assert resp.status_code == 200
        lines = [line for line in resp.iter_lines() if line]
